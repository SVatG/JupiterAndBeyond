...
{
	ivec2_t col1=imat2x2_col1(m),col2=imat2x2_col1(m);
...
{
	int32_t f=idiv(IntToFixed(1),itan(imul(fov,11))); // Rounded.
	return imat4x4perspectiveinternal(
		imul(f,
			isqrt(
				idiv(
					IntToFixed(1),
		...
		imul(f,
			isqrt(
				imul(aspect,aspect) + IntToFixed(1)
			)
		),
		znear,
		zfar
